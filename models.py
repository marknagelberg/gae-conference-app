...
class ConferenceCityQueryForm(messages.Message):
    """ConferenceCityQueryForm -- Conference city query inbound form message"""
...
class SessionTypeFilterForm(messages.Message):
    """SessionTypeFilterForm -- Session type filter inbound form message"""
    typeOfSession = messages.StringField(1)
    websafeConferenceKey = messages.StringField(2)


class SessionNotTypeBeforeTimeForm(messages.Message):
...
class SessionSpeakerFilterForm(messages.Message):
    """SessionForms -- Session speaker fiter inbound form message"""
    speaker = messages.StringField(1)

class SessionDurationFilterForm(messages.Message):
    """SessionDurationFilterForm -- Session duration fiter inbound form message"""
    min = messages.FloatField(1)
    max = messages.FloatField(2)
    websafeConferenceKey = messages.StringField(3)

class SessionWishList(ndb.Model):
    userID = ndb.StringProperty()
    wishlistSessions = ndb.KeyProperty(kind = Session, repeated = True)
...
class ChangeWishList(messages.Message):
    """ChangeWishList -- inbound form message to add or delete from session wishlist."""
